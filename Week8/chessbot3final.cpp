...
namespace nnue{
    ...
    inline int eval(Board &board , Color colour)
    {
        ...
        for(int i=0;i<64;i++)
        {
          ...
          float l1[2*M],l2[N];
          for (int j=0; j<M; j++) {
            l1[j]  = std::min(std::max(0.0f, acc_p1[j]),1.0f);
            l1[M+j] = std::min(std::max(0.0f, acc_p2[j]),1.0f);
        }
        ...
        for(int j=0;j<N;j++)
{
    ...
    for(int k=0;k<2*M;k++)
     ...
     l2[j] = std::min(std::max(0.0f, l2[j]),1.0f);
}
        ...
        final = std::lround(final*1.0f);
       
        return colour==p1?final:-final;
...
int main()
{  
     if (!nnue::load("final3_nnue.bin")) {
        cerr << "info string WARNING: could not load nnue.bin, eval will be all zeros" << endl;
    }
    Board test;
    test.setFen("3r2r1/p1pk1pp1/1pn4p/8/3P4/BNP1P3/P2K1P1P/6R1 w - - 4 22");
    cout<<nnue::eval(test,Color::WHITE);
...
int nn = nnue::eval(board, colour);
int hc = Saksham.board_eval_old();

log << "NNUE        : " << nn << "\n";
log << "Handcrafted : " << hc << "\n";
log << "Difference  : " << (nn - hc) << "\n";
log << "Best move   : " << uci::moveToUci(bestMove) << "\n\n";
